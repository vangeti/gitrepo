		...
		.controller(
				...
				function($scope, $sessionStorage, $state, customwidgetService) {
					...
					/*	$scope.loadData = function() {
							queue().defer(d3.json, "devops/rest/customdefect")
									.await(buildCharts);
						}*/
					...
					$scope.loadData = function() {
						...
						.defer(d3.json, api_url).await(buildCharts);
					}
					...
					$scope.drawChart = function() {
						...
						var chartType = $(
								"input:radio[name='chartType']:checked").val();
						var aggrFunc = $("input:radio[name='aggrFunc']:checked")
								.val();
						...
						buildGraph(chartType, '#chart', dataSet, dimension,
								group, aggrFunc, groupBy, '#total-records',
								xLabel, yLabel);
						$('#customWdiget').show();
					...
					$scope.add2Dashboard = function() {
						...
						if ($scope.widgetName !== '') {
							...
							var dataObj = {
								...
								chart_type : $scope.chartType,
								x_axis : '',
								x_axis_title : $scope.xAxis,
								y_axis : '',
								y_axis_title : $scope.yAxis,
								aggr_fun : $scope.aggregation
							...
							widgetResponse.then(function(data) {
					...
					$('input:radio[name="chartType"]').on('click', function() {
						...
						console.log('chart type: ' + $(this).val());
					...
					/*
					 * $('input:radio[name="chartType"]').change( function(){ if
					 * ($(this).is(':checked') && $(this).val() == 'Yes') { }
					 * });
					 */
