mainApp.controller('CustomWidgetController',
		function($scope, $sessionStorage, $state, customwidgetService) {

			var custResp = customwidgetService.getcustomDefectDetails();

			custResp.then(function(data) {
			...
			queue().defer(d3.json, "devops/rest/customdefect").await(
					buildCharts);
			...
			function buildCharts(error, data) {
				...
				// buildGraph('barChart', '#chart', dataSet, 'Severity',
				// 'Int',
				// '', '', 'count', '#total-records', 'Severity Level',
				// 'Bug
				// Count');
			}
			...
			function drawChart() {
				...
				buildGraph(chartType, '#chart', dataSet, dimension, 'Int', '',
						'', aggrFunc, '#total-records', xLabel, yLabel);

			}
			function buildGraph(chartType, chartId, dataSet, dimension,
					dimDataType, group, groupDataType, aggrFunc,
					total_records_id, xLabel, yLabel) {
				...
				if (aggrFunc == 'count') {
					...
					var objGroup = objDim.group().reduceSum(function(d) {
						return d[group];
					});
				}
				...
				totalRecords.formatNumber(d3.format("d")).valueAccessor(
						function(d) {
							return d;
						}).group(all);

				chart.width(330).height(280).margins({
					top : 10,
					right : 50,
					bottom : 30,
					left : 50
				}).dimension(objDim).group(objGroup);
				...
				if (dimDataType == 'Date') {
					...
					chart.x(d3.scale.ordinal().domain(objDim)).xUnits(
							dc.units.ordinal);
				}

				chart.transitionDuration(500).elasticY(true)
						.renderHorizontalGridLines(true)
						.renderVerticalGridLines(true).yAxisLabel(yLabel)
						.xAxisLabel(xLabel);
				if (chartType == 'barChart') {
					chart.barPadding(0.4).outerPadding(0.2);
				}
				/*
				 * .on('renderlet', function(chart) { }) .on("filtered",
