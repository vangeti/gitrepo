mainApp
		.controller(
				...
				function($scope, $sessionStorage, $rootScope, $http) {
					...
					/*
					 * $http.get("devops/rest/ept").then(function(response) {
					 * 
					 * 
					 * 
					 * var jsonresp = response.data; console.log(jsonresp); for (
					 * var key in jsonresp) { console.log(key + ' error perc ' +
					...
					/*function drawValueComboChart() {
						var data = google.visualization.arrayToDataTable([
								[ 'Build No', 'Error Perc', 'Errors' ],
								[ $scope.keys[0], parseFloat($scope.values[0]), 10.0 ],
								[ $scope.keys[1], parseFloat($scope.values[1]), 10.0 ],
								[ $scope.keys[2], parseFloat($scope.values[2]), 10.0 ],
								[ $scope.keys[3], parseFloat($scope.values[3]), 10.0 ],
								[ $scope.keys[4], parseFloat($scope.values[4]), 10.0 ] ]);
