...
//		$http.get("lennox/rest/hitsPerSec").then(
				function(response) {
					 ...
					 */
				//	console.log("returning valuer:", response);
					...
					for (var key in jsonResponse) {
						var time = secondsToHHMMSS(key);
						arrayObject.push([time, jsonResponse[key]]);
					};
					
					var data = new google.visualization.DataTable();
					data.addColumn('string', 'Elapsed Time');
					data.addColumn('number', '# of Hits');
					data.addRows(arrayObject);
					//var data = new google.visualization.arrayToDataTable(arrayObject);
...
//								format: "HH:mm:ss",
					...
					var chart = new google.visualization.LineChart(document.getElementById('hitsPS'));

					//------------------PDF-------------------------------
					google.visualization.events.addListener(chart, 'ready', function () {
						  var content = '<img src="' + chart.getImageURI() + '">';
						  $('#hitsPerSecondPdf').append(content);
						});
